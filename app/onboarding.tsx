...
export default function OnboardingScreen() {
  ...
  const openAuth = () => router.push({ pathname: "/auth", params: { next: "onboarding", intent: state.accountIntent ?? "personal" } });
  ...
  const completeAccount = () => router.push((isBusiness ? "/business-setup?onboarding=1" : "/personal-details?onboarding=1") as never);
  return <ScreenContainer edges={["top", "bottom", "left", "right"]}><View style={styles.centerContent}><Progress step={3} colors={colors} /><BrandMark colors={colors} /><Text style={[styles.title, { color: colors.foreground }]}>{isAuthenticated ? isBusiness ? "Set up your business." : "Finish your personal profile." : "Join the local conversation."}</Text><Text style={[styles.subtitle, { color: colors.muted }]}>{isAuthenticated ? isBusiness ? "Add the details customers need to discover your business nearby." : "Add a few private essentials before you enter Lekka." : "Browse as a guest, or create your profile to post, connect, and take part in your local community."}</Text>{isAuthenticated ? <Pressable onPress={completeAccount} style={[styles.primaryButton, { backgroundColor: colors.primary }]}><Text style={styles.primaryText}>{isBusiness ? "Set up business" : "Complete profile"}</Text><IconSymbol name="arrow.right" size={19} color="#10211D" /></Pressable> : <><Pressable onPress={openAuth} style={[styles.primaryButton, { backgroundColor: colors.primary }]}><Text style={styles.primaryText}>Create account</Text></Pressable><Pressable onPress={openAuth} style={styles.outlineButton}><Text style={[styles.outlineText, { color: colors.foreground }]}>Sign in</Text></Pressable><Pressable onPress={() => void finishAsGuest()} style={styles.textButton}><Text style={[styles.mutedButtonLabel, { color: colors.muted }]}>Maybe later — continue exploring</Text></Pressable></>}</View></ScreenContainer>;
}
