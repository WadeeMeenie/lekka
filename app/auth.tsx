...
export default function AuthScreen() {
  ...
  const submit = async () => {
    ...
    Alert.alert(mode === "reset" ? "Check your inbox" : mode === "signUp" ? "Account created" : "Welcome back", mode === "reset" ? "A password reset link has been sent if the account exists." : "Your local account is ready.", [{ text: "Continue", onPress: () => router.replace(next === "onboarding" ? "/onboarding?resume=1" : "/(tabs)") }]);
  };
