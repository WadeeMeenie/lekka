...
import { CommunityReport, listCommunityReports, updateCommunityReportStatus } from "@/lib/local-directory";
...
export default function CommunityDetailScreen() {
  ...
  const refreshCommunityData = useCallback(async () => {
    if (!id) return;
    const [membershipResult, membersResult, postsResult, reportsResult] = await Promise.all([
      getCommunityMembershipState(id),
      listCommunityMembers(id),
      listCommunityPosts(id),
      listCommunityReports(id),
    ]);
    ...
    setMembershipError(membershipResult.error?.message || membersResult.error?.message || postsResult.error?.message || reportsResult.error?.message || null);
  ...
  const handleReportStatus = async (report: CommunityReport, status: CommunityReport["status"]) => {
    setMembershipLoading(true);
    setMembershipError(null);
  ...
  return <ScreenContainer edges={["top", "bottom", "left", "right"]}>
    <ScrollView contentContainerStyle={styles.content}>
      ...
      {canModerate && <><View style={styles.sectionHeader}><Text style={[styles.sectionTitle, { color: colors.foreground }]}>Reported content</Text><Text style={[styles.sectionMeta, { color: colors.muted }]}>{reportsLoading ? "Loading…" : `${reports.length} open items`}</Text></View>{reportsLoading ? <ActivityIndicator color={colors.primary} /> : reports.length === 0 ? <Text style={[styles.body, { color: colors.muted }]}>No reports are waiting for review.</Text> : reports.map((report) => <View key={report.id} style={[styles.report, { backgroundColor: colors.surface, borderColor: colors.border }]}><Text style={[styles.reportReason, { color: colors.foreground }]}>{report.reason}</Text><Text style={[styles.reportTarget, { color: colors.muted }]}>{report.comments?.body || report.posts?.body || "Reported community content"}</Text><Text style={[styles.reportMeta, { color: colors.muted }]}>Reported by {report.reporter?.display_name || "a member"} · {report.status}</Text><View style={styles.reportActions}>{(["reviewing", "resolved", "dismissed"] as const).map((status) => <Pressable key={status} accessibilityRole="button" accessibilityLabel={`Mark report ${status}`} onPress={() => void handleReportStatus(report, status)} style={[styles.reportAction, { borderColor: report.status === status ? colors.primary : colors.border, backgroundColor: report.status === status ? `${colors.primary}18` : colors.background }]}><Text style={[styles.reportActionText, { color: report.status === status ? colors.primary : colors.muted }]}>{status}</Text></Pressable>)}</View></View>)}</>}
