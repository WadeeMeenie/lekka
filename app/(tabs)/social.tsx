...
export default function SocialScreen() {
  ...
  return <ScreenContainer><ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
    ...
    {segment === "Communities" ? <>
      ...
      {loading ? <LoadingState colors={colors} /> : error ? <ErrorState colors={colors} message={error} onRetry={load} /> : communities.length === 0 ? <EmptyState colors={colors} title="No communities yet" body="Be the first to start one in your area." action="Create community" onPress={createCommunity} /> : communities.map((community) => <CommunityRow key={community.id} community={community} colors={colors} onPress={() => openCommunity(community)} />)}
    </> : segment === "Following" ? <>
