...
export default function LocalScreen() {
  ...
  const loadDirectory = useCallback(async (category: string) => {
    setLoading(true);
    setError(null);
    try {
  ...
  useEffect(() => { void loadDirectory(selectedCategory); }, [loadDirectory, selectedCategory]);
      ...
      <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
        <View style={styles.headerRow}>
          <View style={styles.headerCopy}>
            <Text style={[styles.title, { color: colors.foreground }]}>Local</Text>
            <Text style={[styles.location, { color: colors.muted }]}>Useful around you</Text>
          </View>
          <View style={[styles.countPill, { backgroundColor: colors.surface, borderColor: colors.border }]}>
            <Text style={[styles.countText, { color: colors.muted }]}>{filteredBusinesses.length}</Text>
          </View>
        </View>
        ...
        <View style={styles.resultHeader}>
          ...
          <Text style={[styles.resultMeta, { color: colors.muted }]}>{filteredBusinesses.length} listings</Text>
        </View>
...
const styles = StyleSheet.create({
  ...
  countPill: { minWidth: 38, height: 32, borderRadius: 16, borderWidth: 1, alignItems: "center", justifyContent: "center", paddingHorizontal: 10 },
  ...
  listing: { minHeight: 72, flexDirection: "row", alignItems: "center", borderWidth: 1, borderRadius: 14, padding: 11, marginBottom: 8 },
  ...
  listingMeta: { fontSize: 12, lineHeight: 16, marginTop: 1 },
  listingDescription: { fontSize: 12, lineHeight: 16, marginTop: 1 },
