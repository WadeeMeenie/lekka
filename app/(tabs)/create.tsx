...
export default function CreateScreen() {
  ...
  const publish = async () => {
    ...
    const draft: LocalPost = { id: `draft-${Date.now()}`, kind, category: type === "Local alert" ? "Alert" : undefined, author: "You · Offline draft", initials: "YO", area, distance: liveLocation ? "Current area" : "Nearby", time: "saved now", title: type === "Local alert" ? "Community alert" : undefined, body: text.trim(), likes: 0, comments: 0, trusted: false, accent: type === "Local alert" ? "#D95D4F" : "#2F7D67" };
    await savePosts([draft, ...existing]); setText(""); setPublishing(false); Alert.alert("You’re offline", "Your post is saved as a draft and will be ready to publish when you’re back online.");
  };
