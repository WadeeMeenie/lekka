...
export default function HomeScreen() {
  ...
  return <ScreenContainer containerClassName="bg-background">
    ...
    />
    {sheet && <PostActionSheet sheet={sheet} colors={colors} isOwner={user?.id === sheet.post?.authorId} onClose={()=>setSheet(null)} onNotInterested={(postId)=>void submitFeedback(postId,"not_interested")} onReact={(postId,reaction)=>void react(postId,reaction)} onCopyLink={(post)=>{void Clipboard.setStringAsync(`manuslocalradarsa://post/${post.id}`); setSheet(null); setToast("Post link copied.");}} onShareExternal={(post)=>{setSheet(null); void Share.share({message:`${post.title ? `${post.title}\n` : ""}${post.body}\n\nShared from Lekka`});}} onShareAsPost={(post)=>void shareAsPost(post)} onDelete={(postId)=>void deletePost(postId)} onReport={(postId)=>void report(postId)} onEdit={(postId)=>{setSheet(null); router.push({pathname:"/edit-post/[id]",params:{id:postId}} as never);}} />}
...
function PostActionSheet({sheet,colors,isOwner,onClose,onNotInterested,onReact,onCopyLink,onShareExternal,onShareAsPost,onDelete,onReport,onEdit}:{sheet:{type:"post"|"reaction"|"share"|"delete";post?:LocalPost};colors:ReturnType<typeof useColors>;isOwner:boolean;onClose:()=>void;onNotInterested:(postId:string)=>void;onReact:(postId:string,reaction:ReactionType)=>void;onCopyLink:(post:LocalPost)=>void;onShareExternal:(post:LocalPost)=>void;onShareAsPost:(post:LocalPost)=>void;onDelete:(postId:string)=>void;onReport:(postId:string)=>void;onEdit:(postId:string)=>void}) {
