...
export default function HomeScreen() {
  ...
  return <ScreenContainer containerClassName="bg-background">
    ...
    <FlatList data={visiblePosts} keyExtractor={(item) => item.id} onEndReached={loadMore} onEndReachedThreshold={0.55} ListFooterComponent={loadingMore ? <ActivityIndicator color={colors.primary} style={{ marginVertical: 18 }} /> : null} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={refresh} tintColor={colors.primary} />} contentContainerStyle={styles.content}
      ...
      renderItem={({ item }) => <PostCard post={item} colors={colors} feedback={feedback[item.id]} reaction={reactions[item.id]} busy={busyPostIds.has(item.id)} profileImageUrl={profileImageUrls[item.id]} postImageUrl={postImageUrls[item.id]} onFeedback={(preference) => void submitFeedback(item.id, preference)} onOpen={() => router.push({ pathname: "/post/[id]", params: { id: item.id } } as never)} onLike={() => void react(item.id)} onLongReact={() => chooseReaction(item.id)} onSave={() => void save(item.id)} onShare={() => void share(item)} onProtectedAction={(action) => { if (!isAuthenticated) setAuthGateAction(action); }} />}
...
function PostCard({ post, colors, feedback, reaction, busy, profileImageUrl, postImageUrl, onFeedback, onOpen, onLike, onLongReact, onSave, onShare, onProtectedAction }: { post: LocalPost; colors: ReturnType<typeof useColors>; feedback?: FeedPreference; reaction?: ReactionType | null; busy: boolean; profileImageUrl?: string; postImageUrl?: string; onFeedback: (preference: FeedPreference) => void; onOpen: () => void; onLike: () => void; onLongReact: () => void; onSave: () => void; onShare: () => void; onProtectedAction: (action: string) => void }) {
  const openMenu = () => Alert.alert("Post options", undefined, [
    { text: "Not interested", onPress: () => onFeedback("not_interested") },
    { text: "Cancel", style: "cancel" },
  ]);
