...
export default function HomeScreen() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      if (user) {
        ...
        try { const sync = await syncPendingPostDrafts(user.id); const remaining = sync.remaining ?? (await listPendingPostDrafts(user.id)).length; if (active) { setPendingDrafts(remaining); setOutboxStatus(remaining > 0 ? "queued" : "idle"); } } catch { if (active) { setPendingDrafts((await listPendingPostDrafts(user.id)).length); setOutboxStatus("error"); } }
      ...
      if (liveLocation) { setDeviceLocation(liveLocation); setArea(liveLocation.area); } else setArea(settings.area);
      const firstPage = await fetchFeedPage(liveLocation, null);
      setPosts(firstPage.posts); setNextCursor(firstPage.nextCursor); if (active) setInitialLoading(false);
    })().catch(() => { if (active) setInitialLoading(false); });
    const unsubscribe = subscribeToLocalChanges(() => { setBackgroundRefreshing(true); void fetchFeedPage(deviceLocation ?? undefined, null).then((next) => { if (active) { setPosts(next.posts); setNextCursor(next.nextCursor); } }).finally(() => { if (active) setBackgroundRefreshing(false); }); });
  ...
  useEffect(() => {
    ...
    const resolveImages = async () => {
      const profileEntries = posts.filter((post) => post.profileImagePath).map((post) => [post.id, post.profileImagePath as string] as const);
      const mediaEntries = posts.filter((post) => post.mediaPath).map((post) => [post.id, post.mediaPath as string] as const);
      const profilePairs = await Promise.all(profileEntries.map(async ([id, path]) => {
        const { data } = await createSignedMediaUrl(path);
        return data?.signedUrl ? [id, data.signedUrl] as const : null;
      }));
      const mediaPairs = await Promise.all(mediaEntries.map(async ([id, path]) => {
        const { data } = await createSignedMediaUrl(path);
        return data?.signedUrl ? [id, data.signedUrl] as const : null;
      }));
      if (!active) return;
      setProfileImageUrls(Object.fromEntries(profilePairs.filter(Boolean) as Array<readonly [string, string]>) as Record<string, string>);
      setPostImageUrls(Object.fromEntries(mediaPairs.filter(Boolean) as Array<readonly [string, string]>) as Record<string, string>);
    };
    void resolveImages();
    return () => { active = false; };
  }, [posts]);
  ...
  const submitFeedback = async (postId: string, preference: FeedPreference) => {
    ...
    if (result.error) { setFeedback((current) => { const next = { ...current }; if (previous) next[postId] = previous; else delete next[postId]; return next; }); return; }
    if (preference === "not_interested") Alert.alert("Not interested", "You won't see posts like this in future.");
  };
  ...
  const chooseReaction = (postId: string) => {
    Alert.alert("React to this post", "Choose your reaction", [
      ...REACTION_OPTIONS.map((emoji) => ({ text: emoji, onPress: () => void react(postId, emoji) })),
  ...
  const deletePost = async (postId: string) => {
    ...
    if (result.error || !result.deleted) {
      Alert.alert("Couldn't delete post", result.error?.message || "The post could not be deleted. Please try again.");
      return;
    }
    setPosts((current) => current.filter((post) => post.id !== postId));
    setFeedback((current) => { const next = { ...current }; delete next[postId]; return next; });
    setReactions((current) => { const next = { ...current }; delete next[postId]; return next; });
    setProfileImageUrls((current) => { const next = { ...current }; delete next[postId]; return next; });
    setPostImageUrls((current) => { const next = { ...current }; delete next[postId]; return next; });
  };

  const confirmDeletePost = (postId: string) => Alert.alert("Delete post?", "This can't be undone.", [
    { text: "Cancel", style: "cancel" },
...
function PostCard({ post, colors, feedback, reaction, busy, profileImageUrl, postImageUrl, isOwner, onFeedback, onOpen, onLike, onLongReact, onSave, onShare, onDelete, onProtectedAction }: { post: LocalPost; colors: ReturnType<typeof useColors>; feedback?: FeedPreference; reaction?: ReactionType | null; busy: boolean; profileImageUrl?: string; postImageUrl?: string; isOwner: boolean; onFeedback: (preference: FeedPreference) => void; onOpen: () => void; onLike: () => void; onLongReact: () => void; onSave: () => void; onShare: () => void; onDelete: () => void; onProtectedAction: (action: string) => void }) {
  const openMenu = () => {
    const options = [];
