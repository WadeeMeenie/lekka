...
import { checkUsernameAvailability, findAvailableUsernameSuggestions, normalizeUsername, USERNAME_MAX_LENGTH, USERNAME_MIN_LENGTH, validateUsername } from "@/lib/username";
...
export default function ProfileScreen() {
  ...
  useEffect(() => {
    if (!user) return;
    void loadUsernameHistory(user.id).then((history) => { setUsernameHistory(history); setUsernameCooldown(usernameCooldownMessage(history)); });
  }, [user]);
  useEffect(() => {
  ...
  const checkUsername = async () => {
    ...
    else setUsernameSuggestions(await findAvailableUsernameSuggestions(username, user?.id));
