...
import { PROFILE_AVATAR_UPDATED_MESSAGE } from "@/lib/profile-avatar";
...
export default function ProfileScreen() {
  ...
  const showAvatarToast = () => {
    ...
    setAvatarToast(PROFILE_AVATAR_UPDATED_MESSAGE);
    void AccessibilityInfo.announceForAccessibility(PROFILE_AVATAR_UPDATED_MESSAGE);
    avatarToastTimer.current = setTimeout(() => setAvatarToast(null), 2800);
  };
  ...
  const pickAvatar = async () => {
    try {
      ...
      setFeedback({ tone: "error", message: "Lekka couldn’t open your photo library. You can keep using your initials avatar." });
    }
  };
  ...
  const save = async (confirmedUsernameChange = false) => {
    setFeedback(null);
    ...
    if (cooldown) {
      setFeedback({ tone: "error", message: cooldown });
      return;
    }
    if (Object.keys(errors).length > 0 || !usernameAvailable) {
      setFeedback({ tone: "error", message: errors.displayName ?? errors.bio ?? "Choose an available username before saving." });
      return;
    }
    if (usernameChanged && !confirmedUsernameChange) {
      ...
      if (avatarUri) {
        ...
        try {
          ...
          if (avatarResult.error) {
            setFeedback({ tone: "error", message: "Your details were saved, but the avatar could not upload. Check your connection and try again." });
    ...
    if (result.error) setFeedback({ tone: "error", message: duplicateUsername ? "Choose another username and try again." : user ? "We couldn't save your changes. Check your connection and try again." : "Create an account to save a profile." });
    else if (!avatarUri) setFeedback({ tone: "success", message: "Profile updated. Your Lekka profile is ready for the local world." });
  };
