...
export default function MessagesScreen() {
  ...
  const loadConversations = async () => {
    ...
    try {
      ...
      if (result.error) {
        setLoadError(result.error.message || "We couldn't load your messages. Please try again.");
        return;
      }
      setConversations(result.data);
      if (conversationId) setSelected(result.data.find((conversation) => conversation.id === conversationId) ?? null);
    } catch (error) {
      setLoadError(error instanceof Error ? error.message : "We couldn't load your messages. Please try again.");
    } finally {
      setLoading(false);
    }
  };
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        if (active) Alert.alert("Couldn't load conversation", error instanceof Error ? error.message : "Please try again.");
      }
    };
    void load();
  ...
  if (selected) {
    ...
    const respond = async (status: "accepted" | "rejected") => {
      ...
      try {
        ...
        Alert.alert("Message request failed", error instanceof Error ? error.message : "Please try again.");
