...
export default function PostDetailScreen() {
  ...
  <Modal visible={optionsOpen} transparent animationType="slide" onRequestClose={() => !actionBusy && setOptionsOpen(false)}>
    <View style={styles.modalBackdrop}><View style={[styles.sheet, { backgroundColor: colors.surface }]}>
      ...
      {actionError && <Text style={[styles.sheetError, { color: colors.danger ?? "#E5484D" }]}>{actionError}</Text>}
      <Pressable onPress={() => setOptionsOpen(false)} disabled={actionBusy} style={styles.sheetRow}><IconSymbol name="xmark.circle" size={20} color={colors.muted} /><Text style={[styles.sheetText, { color: colors.foreground }]}>Cancel</Text></Pressable>
      <Pressable onPress={reportPost} disabled={actionBusy} style={styles.sheetRow}><IconSymbol name="flag.fill" size={20} color={colors.muted} /><Text style={[styles.sheetText, { color: colors.foreground }]}>{actionBusy ? "Working…" : "Report post"}</Text></Pressable>
      {isOwner && <Pressable onPress={() => { setOptionsOpen(false); setDeleteOpen(true); setActionError(null); }} disabled={actionBusy} style={styles.sheetRow}><IconSymbol name="trash.fill" size={20} color={colors.danger ?? "#E5484D"} /><Text style={[styles.sheetText, { color: colors.danger ?? "#E5484D" }]}>Delete post</Text></Pressable>}
    </View></View>
  </Modal>
