...
export default function PostDetailScreen() {
  ...
  const applyReaction = (nextReaction: ReactionType = "👍") => runProtected("react to posts", async () => {
    ...
    setReactionBusy(true); setReactionPicker(false);
    try {
      const result = await toggleReactionAtomic(id, nextReaction);
      if (result.error) { Alert.alert("Couldn't update reaction", result.error.message || "Please try again."); return; }
      setReaction(result.reaction); setLiked(Boolean(result.reaction));
      const refreshed = await getPostInteractionState(id);
      if (!refreshed.error && refreshed.data) { setReactionCount(refreshed.data.reactions); setSaved(refreshed.data.saved); setLiked(refreshed.data.liked); }
    } finally { setReactionBusy(false); }
  });

  const save = () => runProtected("save posts", async () => {
    if (saveBusy) return; setSaveBusy(true);
    try { const result = await toggleSavedPostAtomic(id); if (result.error) { Alert.alert("Couldn't update saved posts", result.error.message || "Please try again."); return; } setSaved(result.saved); } finally { setSaveBusy(false); }
  });

  const submitComment = async () => {
    if (!isAuthenticated) { setAuthGateAction("comment on posts"); return; }
    if (commentBusy || !commentText.trim()) return;
    setCommentBusy(true);
    try { const result = await createCommentSafe(id, commentText); if (result.error || !result.data) { Alert.alert("Couldn't comment", result.error?.message || "Please try again."); return; } setComments((current) => [result.data!, ...current]); setCommentCount((current) => current + 1); setCommentText(""); }
    finally { setCommentBusy(false); }
  };

  const sharePost = async () => {
    const title = post?.title ?? "Lekka post";
  ...
  return <ScreenContainer edges={["top", "bottom", "left", "right"]}><ScrollView ref={scrollRef} contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
    ...
    {post.title && <Text style={[styles.title, { color: colors.foreground }]}>{post.title}</Text>}<Text style={[styles.body, { color: colors.foreground }]}>{post.body}</Text>{mediaUrls.map((url) => <Image key={url} source={{ uri: url }} style={styles.media} resizeMode="cover" />)}
