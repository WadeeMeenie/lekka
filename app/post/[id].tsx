...
import { createSignedMediaUrl, createComment, deleteComment, getPostDetail, getPostInteractionState, listComments, reportContent, toggleReaction, toggleSavedPost, type SocialComment, type SocialPost } from "@/lib/social-repository";
...
export default function PostDetailScreen() {
  ...
  const reportPost = () => runProtected("report posts", async () => { const result = await reportContent({ postId: id, reason: "User report" }); Alert.alert(result.error ? "Couldn’t report" : "Report received", result.error ? "Please try again." : "Thanks. The report has been recorded for review."); });
