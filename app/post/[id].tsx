...
export default function PostDetailScreen() {
  ...
  const submitComment = async () => { if (!isAuthenticated) { setAuthGateAction("comment on posts"); return; } const trimmedComment = commentText.trim(); if (commentBusy || !trimmedComment) return; setCommentBusy(true); setFeedback(null); try { const result = await createCommentSafe(id, trimmedComment); if (result.error || !result.data) { setFeedback(result.error?.message || "Couldn't comment. Please try again."); return; } setComments((current) => [result.data!, ...current]); setCommentCount((current) => current + 1); setCommentText(""); requestAnimationFrame(() => scrollRef.current?.scrollToEnd({ animated: true })); } finally { setCommentBusy(false); } };
  ...
  const removeComment = async (commentId: string) => { if (deletingCommentId) return; setDeletingCommentId(commentId); setFeedback(null); try { const result = await deleteComment(commentId); if (result.error) { setFeedback("Couldn't delete comment. Please try again."); return; } setComments((current) => current.filter((comment) => comment.id !== commentId)); setCommentCount((current) => Math.max(0, current - 1)); } finally { setDeletingCommentId(null); } };
  ...
  return <ScreenContainer edges={["top", "bottom", "left", "right"]}><KeyboardAvoidingView style={styles.keyboardContainer} behavior={Platform.OS === "ios" ? "padding" : "height"}><ScrollView ref={scrollRef} contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled" keyboardDismissMode="on-drag">
    ...
    <View style={[styles.actions, { borderColor: colors.border }]}><Pressable disabled={reactionBusy} onPress={() => applyReaction(reaction ?? "👍")} onLongPress={() => { if (isAuthenticated) setReactionPicker(true); else setAuthGateAction("react to posts"); }} delayLongPress={350} style={styles.action} accessibilityRole="button" accessibilityLabel={reaction ? `Remove reaction, ${reactionCount} reactions` : `React to post, ${reactionCount} reactions`} accessibilityState={{ disabled: reactionBusy, busy: reactionBusy }}>{reaction ? <Text style={styles.selectedEmoji}>{reaction}</Text> : <IconSymbol name="heart.fill" size={20} color={colors.muted} />}<Text style={[styles.actionText, { color: colors.muted }]}>{reactionCount}</Text></Pressable><Pressable onPress={() => scrollRef.current?.scrollToEnd({ animated: true })} style={styles.action} accessibilityRole="button" accessibilityLabel={`${commentCount} comments`}><IconSymbol name="bubble.left.fill" size={20} color={colors.muted} /><Text style={[styles.actionText, { color: colors.muted }]}>{commentCount}</Text></Pressable><Pressable disabled={saveBusy} onPress={save} style={styles.action} accessibilityRole="button" accessibilityLabel={saved ? "Remove post from saved" : "Save post"} accessibilityState={{ disabled: saveBusy, busy: saveBusy }}><IconSymbol name="bookmark.fill" size={20} color={saved ? colors.primary : colors.muted} /><Text style={[styles.actionText, { color: colors.muted }]}>{saved ? "Saved" : "Save"}</Text></Pressable><Pressable onPress={sharePost} style={styles.action} accessibilityRole="button" accessibilityLabel="Share post"><IconSymbol name="square.and.arrow.up" size={20} color={colors.muted} /><Text style={[styles.actionText, { color: colors.muted }]}>Share</Text></Pressable></View>
    <Text style={[styles.sectionTitle, { color: colors.foreground }]}>Comments</Text>
    <View style={[styles.commentComposer, { backgroundColor: colors.surface, borderColor: colors.border }]}><TextInput ref={commentInputRef} value={commentText} onChangeText={setCommentText} placeholder={isAuthenticated ? "Say something useful…" : "Join Lekka to comment"} placeholderTextColor={colors.muted} editable={isAuthenticated && !commentBusy} multiline maxLength={500} textAlignVertical="top" returnKeyType="default" onFocus={() => requestAnimationFrame(() => scrollRef.current?.scrollToEnd({ animated: true }))} style={[styles.commentInput, { color: colors.foreground }]} accessibilityLabel="Comment" accessibilityHint="Write a comment of up to 500 characters" /><Pressable onPress={submitComment} disabled={commentBusy || !isAuthenticated || !commentText.trim()} style={[styles.send, { backgroundColor: colors.primary, opacity: commentBusy || !isAuthenticated || !commentText.trim() ? 0.55 : 1 }]} accessibilityRole="button" accessibilityLabel={commentBusy ? "Posting comment" : "Submit comment"} accessibilityState={{ disabled: commentBusy || !isAuthenticated || !commentText.trim(), busy: commentBusy }}><IconSymbol name="arrow.up.right" size={17} color="#10211D" /></Pressable></View>
    {comments.length === 0 ? <Text style={[styles.empty, { color: colors.muted }]}>Be the first to say something.</Text> : comments.map((comment) => <View key={comment.id} style={[styles.comment, { borderBottomColor: colors.border }]}><View style={[styles.commentAvatar, { backgroundColor: colors.primary }]}>{commentAvatarUrls[comment.id] ? <Image source={{ uri: commentAvatarUrls[comment.id] }} contentFit="cover" style={styles.commentAvatarImage} /> : <Text style={styles.commentInitials}>{(comment.profiles?.display_name ?? "LN").slice(0, 2).toUpperCase()}</Text>}</View><View style={styles.commentCopy}><Text style={[styles.commentAuthor, { color: colors.foreground }]}>{comment.profiles?.display_name ?? "Local neighbour"}</Text><Text style={[styles.commentBody, { color: colors.foreground }]}>{comment.body}</Text><Text style={[styles.commentTime, { color: colors.muted }]}>{formatPostTime(comment.created_at)}</Text></View>{user?.id === comment.author_id && <Pressable onPress={() => void removeComment(comment.id)} disabled={deletingCommentId === comment.id || deletingCommentId !== null} style={styles.commentDelete} accessibilityRole="button" accessibilityLabel={deletingCommentId === comment.id ? "Deleting comment" : "Delete comment"} accessibilityState={{ disabled: deletingCommentId === comment.id || deletingCommentId !== null, busy: deletingCommentId === comment.id }}><IconSymbol name="trash.fill" size={17} color={colors.muted} /></Pressable>}</View>)}
  </ScrollView>
