...
export default function MenuScreen() {
  ...
  useFocusEffect(useCallback(() => {
    let active = true;
    if (!user) { setDisplayName(null); setUsername(null); return () => { active = false; }; }
    void loadMyProfile().then(({ data }) => { if (active) { setDisplayName(data?.display_name ?? null); setUsername(data?.username ?? null); } });
    return () => { active = false; };
  }, [user?.id]));

  const signOut = () => Alert.alert("Sign out of Lekka?", "You can sign back in whenever you are ready.", [
    { text: "Cancel", style: "cancel" },
    { text: "Sign out", style: "destructive", onPress: () => { void logout().then(() => router.replace("/auth" as never)); } },
  ]);

  return <ScreenContainer edges={["top", "bottom", "left", "right"]}><ScrollView contentContainerStyle={styles.content}><View style={styles.header}><Pressable accessibilityLabel="Go back" onPress={() => router.back()} style={styles.back}><IconSymbol name="chevron.right" size={23} color={colors.foreground} style={styles.backIcon} /></Pressable><Text style={[styles.title, { color: colors.foreground }]}>Menu</Text></View><Pressable accessibilityRole="button" onPress={() => router.push(isAuthenticated ? "/profile" as never : "/auth" as never)} style={[styles.profileCard, { backgroundColor: colors.surface, borderColor: colors.border }]}><View style={[styles.avatar, { backgroundColor: colors.primary }]}><Text style={styles.avatarText}>{getAvatarInitials(displayName)}</Text></View><View style={styles.profileCopy}><Text style={[styles.profileName, { color: colors.foreground }]}>{isAuthenticated ? displayName || "Your Lekka profile" : "Join Lekka"}</Text><Text style={[styles.profileDetail, { color: colors.muted }]}>{isAuthenticated ? username ? `@${username}` : "View and edit your profile" : "Sign in to personalize your local network"}</Text></View><IconSymbol name="chevron.right" size={22} color={colors.muted} /></Pressable><Text style={[styles.sectionLabel, { color: colors.muted }]}>YOUR LEKKA</Text><View style={styles.grid}>{menuItems.map((item) => <Pressable key={item.title} accessibilityRole="button" accessibilityLabel={item.title} onPress={() => router.push(item.route as never)} style={({ pressed }) => [styles.menuCard, { backgroundColor: colors.surface, borderColor: colors.border }, pressed && styles.pressed]}><View style={[styles.iconTile, { backgroundColor: `${colors.primary}18` }]}><IconSymbol name={item.icon} size={22} color={colors.primary} /></View><Text style={[styles.menuTitle, { color: colors.foreground }]}>{item.title}</Text><Text numberOfLines={2} style={[styles.menuDetail, { color: colors.muted }]}>{item.detail}</Text></Pressable>)}</View><Text style={[styles.sectionLabel, { color: colors.muted }]}>SUPPORT</Text><Pressable onPress={() => router.push("/feedback" as never)} style={[styles.supportCard, { borderColor: colors.border }]}><IconSymbol name="bubble.left.and.bubble.right.fill" size={20} color={colors.primary} /><View style={styles.supportCopy}><Text style={[styles.supportTitle, { color: colors.foreground }]}>Report a problem</Text><Text style={[styles.supportDetail, { color: colors.muted }]}>Send a bug report or feature request from inside Lekka.</Text></View><IconSymbol name="chevron.right" size={20} color={colors.muted} /></Pressable>{isAuthenticated && <Pressable accessibilityRole="button" onPress={signOut} style={styles.signOut}><IconSymbol name="close" size={18} color={colors.error} /><Text style={[styles.signOutText, { color: colors.error }]}>Sign out</Text></Pressable>}</ScrollView></ScreenContainer>;
}

const styles = StyleSheet.create({ content: { padding: 20, paddingBottom: 42 }, header: { flexDirection: "row", alignItems: "center", gap: 10, marginBottom: 21 }, back: { width: 34, height: 34, alignItems: "center", justifyContent: "center" }, backIcon: { transform: [{ rotate: "180deg" }] }, title: { fontSize: 29, fontWeight: "900" }, profileCard: { minHeight: 86, borderWidth: 1, borderRadius: 18, padding: 13, flexDirection: "row", alignItems: "center", gap: 12 }, avatar: { width: 52, height: 52, borderRadius: 18, alignItems: "center", justifyContent: "center" }, avatarText: { color: "#10211D", fontSize: 16, fontWeight: "900" }, profileCopy: { flex: 1 }, profileName: { fontSize: 16, fontWeight: "900" }, profileDetail: { fontSize: 12, marginTop: 4 }, sectionLabel: { fontSize: 10, fontWeight: "900", letterSpacing: 1.1, marginTop: 24, marginBottom: 10 }, grid: { flexDirection: "row", flexWrap: "wrap", gap: 10 }, menuCard: { width: "48.5%", minHeight: 144, borderWidth: 1, borderRadius: 17, padding: 13 }, iconTile: { width: 38, height: 38, borderRadius: 13, alignItems: "center", justifyContent: "center" }, menuTitle: { fontSize: 15, fontWeight: "900", marginTop: 15 }, menuDetail: { fontSize: 11, lineHeight: 16, marginTop: 5 }, supportCard: { borderWidth: 1, borderRadius: 17, padding: 14, flexDirection: "row", alignItems: "center", gap: 11 }, supportCopy: { flex: 1 }, supportTitle: { fontSize: 14, fontWeight: "900" }, supportDetail: { fontSize: 11, lineHeight: 16, marginTop: 3 }, signOut: { minHeight: 48, flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 8, marginTop: 23 }, signOutText: { fontSize: 14, fontWeight: "900" }, pressed: { opacity: 0.76, transform: [{ scale: 0.985 }] } });
