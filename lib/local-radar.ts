...
export type LocalPost = { id: string; kind: PostKind; category?: RadarCategory; author: string; initials: string; area: string; distance: string; time: string; title?: string; body: string; likes: number; comments: number; trusted: boolean; accent: string };
...
export function personalizeFeed(posts: LocalPost[], tab: FeedTab, feedback: Record<string, FeedPreference>): LocalPost[] { const filtered = posts.filter((post) => feedback[post.id] !== "not_interested"); return rankPosts(filtered, tab); }
export async function loadSettings(): Promise<LocalSettings> { try { const raw = await AsyncStorage.getItem("lekka.local.settings"); return raw ? { ...defaultSettings, ...JSON.parse(raw) } : defaultSettings; } catch { return defaultSettings; } }
export async function saveSettings(settings: LocalSettings) { await AsyncStorage.setItem("lekka.local.settings", JSON.stringify(settings)); }
export async function loadPosts(): Promise<LocalPost[]> { try { const raw = await AsyncStorage.getItem("lekka.local.posts"); return raw ? JSON.parse(raw) : seededPosts; } catch { return seededPosts; } }
export async function savePosts(posts: LocalPost[]) { await AsyncStorage.setItem("lekka.local.posts", JSON.stringify(posts)); }
