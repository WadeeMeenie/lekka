import { getLastKnownOrCurrentLocation } from "@/lib/location";
...
export async function getPostDetailSafe(postId: string) {
  if (!supabase) return { data: null as SocialPost | null, error: backendError(), authorizedMediaUrls: {} as Record<string, string> };

  const direct = await supabase.from("posts").select(POST_SELECT).eq("id", postId).maybeSingle();
  if (direct.error || direct.data) {
    return { data: direct.data as unknown as SocialPost | null, error: direct.error, authorizedMediaUrls: {} as Record<string, string> };
  }

  // Nearby posts are intentionally absent from the direct table policy. Resolve
  // them through the authenticated Edge Function, which performs location
  // authorization before returning short-lived signed media URLs.
  const locationResult = await getLastKnownOrCurrentLocation();
  if (locationResult.status !== "granted") {
    return { data: null as SocialPost | null, error: null, authorizedMediaUrls: {} as Record<string, string> };
  }

  const { data, error } = await supabase.functions.invoke("nearby-post-detail", {
    body: {
      postId,
  ...
  if (error || !data) {
    return {
      data: null as SocialPost | null,
      error: error ?? new Error("This post is outside your authorized discovery area"),
      authorizedMediaUrls: {} as Record<string, string>,
    };
  }

  const payload = data as NearbyPostDetailResponse;
  return {
    data: payload.data.post,
    error: null,
    authorizedMediaUrls: payload.data.mediaUrls ?? {},
  };
}
