...
export async function getLastKnownOrCurrentLocation(fallbackArea = "Bellville"): Promise<LocationResult> {
  if (Platform.OS === "web") return { status: "unavailable", area: fallbackArea };
  const permission = await Location.getForegroundPermissionsAsync();
  if (permission.status !== Location.PermissionStatus.GRANTED) return requestApproximateLocation(fallbackArea);
...
export async function watchMeaningfulForegroundLocation(
  ...
  const subscription = await Location.watchPositionAsync(
    {
      accuracy: Location.Accuracy.Balanced,
      distanceInterval: MOVEMENT_REFRESH_METERS,
      timeInterval: LOCATION_REFRESH_INTERVAL_MS,
      mayShowUserSettingsDialog: false,
    },
    (position) => {
      void toDeviceLocation(position, fallbackArea).then(onLocation);
    },
  );
