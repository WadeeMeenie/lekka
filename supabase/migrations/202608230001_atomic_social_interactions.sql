...
-- SECURITY INVOKER is intentional: RLS remains the authorization boundary.
...
begin
  ...
  if not exists (
    ...
    where p.id = p_post_id
      and (p.visibility = 'public' or p.author_id = v_user_id or (p.visibility = 'nearby' and v_user_id is not null))
  ) then
  ...
  select exists (select 1 from public.reactions where post_id = p_post_id and user_id = v_user_id) into v_exists;
  ...
  insert into public.reactions(post_id, user_id, reaction) values (p_post_id, v_user_id, 'like');
  return true;
end;
$$;
...
begin
  ...
  select exists (select 1 from public.saved_posts where post_id = p_post_id and user_id = v_user_id) into v_exists;

  if v_exists then
    delete from public.saved_posts where post_id = p_post_id and user_id = v_user_id;
    return false;
  end if;

  insert into public.saved_posts(post_id, user_id) values (p_post_id, v_user_id);
  return true;
end;
$$;
...
begin
  ...
  select exists (select 1 from public.follows where follower_id = v_user_id and following_id = p_profile_id) into v_exists;

  if v_exists then
    delete from public.follows where follower_id = v_user_id and following_id = p_profile_id;
    return false;
  end if;

  insert into public.follows(follower_id, following_id) values (v_user_id, p_profile_id);
  return true;
end;
$$;
