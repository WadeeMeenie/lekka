...
-- Communities behave like followed groups. Existing membership is the source of truth.
create index if not exists community_members_user_joined_idx
  on public.community_members (user_id, joined_at desc);

-- Ensure users cannot accidentally create duplicate membership rows.
create unique index if not exists community_members_unique_idx
  on public.community_members (community_id, user_id);

-- Public community discovery remains readable; private communities are readable to members/owners.
...
-- Community membership is private to the user and community moderators/owners.
...
create policy "members can read own community membership"
  ...
  using (
    ...
    or exists (select 1 from public.communities c where c.id = community_id and c.created_by = (select auth.uid()))
    or exists (select 1 from public.community_members cm where cm.community_id = community_id and cm.user_id = (select auth.uid()) and cm.is_moderator = true)
  );

-- Keep community creation server-owned by the authenticated creator.
drop policy if exists "authenticated users can create communities" on public.communities;
create policy "authenticated users can create communities"
  on public.communities for insert to authenticated
  with check ((select auth.uid()) = created_by);

-- Community posts must belong to a community the author can participate in.
...
create policy "community members can create community posts"
  ...
  with check (
    ...
    and (
      ...
      or exists (select 1 from public.community_members cm where cm.community_id = community_id and cm.user_id = (select auth.uid()))
    )
  );
