...
create index post_feedback_user_preference_idx on public.post_feedback(user_id, preference, updated_at desc);

alter table public.post_feedback enable row level security;
create policy post_feedback_self_access on public.post_feedback
...
create index buddy_requests_sender_idx on public.buddy_requests(sender_id, status, updated_at desc);
create index buddy_requests_recipient_idx on public.buddy_requests(recipient_id, status, updated_at desc);

alter table public.buddy_requests enable row level security;
create policy buddy_requests_participant_read on public.buddy_requests
...
begin
  ...
  if found and result.status = 'pending' then
    update public.buddy_requests set status = 'accepted', updated_at = now() where id = result.id returning * into result;
    return result;
  end if;
  if found and result.status = 'accepted' then return result; end if;

  insert into public.buddy_requests (sender_id, recipient_id) values (auth.uid(), p_recipient_id)
...
begin
  ...
  update public.buddy_requests set status = p_status, updated_at = now()
...
begin
  ...
  where status in ('pending', 'accepted')
    and ((sender_id = auth.uid() and recipient_id = p_profile_id) or (sender_id = p_profile_id and recipient_id = auth.uid()));
end;
$$;

revoke all on function public.request_buddy(uuid) from public;
revoke all on function public.respond_to_buddy_request(uuid, text) from public;
