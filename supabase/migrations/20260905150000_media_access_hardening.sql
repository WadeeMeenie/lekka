...
create policy media_authenticated_read
  ...
  using (
    ...
    and (
      ...
      or exists (
        select 1 from public.profiles profile where profile.profile_image_path = storage.objects.name
      )
    )
  );
...
begin
  ...
  if post_author is null then raise exception 'Post media must reference an existing post'; end if;
  if (storage.foldername(new.storage_path))[1] is distinct from post_author::text then raise exception 'Post media must belong to the post author storage namespace'; end if;
  return new;
end;
$$;
