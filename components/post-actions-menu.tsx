...
const POST_LINK_SCHEME = "manuslocalradarsa";
...
export function PostActionsMenu({
  ...
  const postLink = `${POST_LINK_SCHEME}://post/${postId}`;
  ...
  const sharePost = async () => {
    setVisible(false);
    await Share.share({
      message: `Check out this post on Lekka:\n${postLink}`,
    });
  };
  ...
  const actions: Action[] = isOwner
    ? [
        ...
        ...(onMuted
          ? [{ label: "Turn off notifications", icon: "notifications-off" as const, onPress: () => { setVisible(false); onMuted(); } }]
          : []),
        {
          label: "Delete post",
          icon: "delete-outline",
    ...
    : [
        ...
        ...(onMuted
          ? [{ label: "Mute user", icon: "volume-off" as const, onPress: () => { setVisible(false); onMuted(); } }]
          : []),
        {
          label: "Report post",
          icon: "flag",
        ...
        { label: "Copy link", icon: "link", onPress: () => void copyLink() },
        { label: "Share post", icon: "ios-share", onPress: () => void sharePost() },
      ];
        ...
        <Pressable style={styles.scrim} onPress={close}>
          ...
          >
            ...
            <Text style={[styles.title, { color: colors.foreground }]}>
              {confirmDelete ? "Delete this post?" : "Post options"}
            </Text>
            ...
            {confirmDelete ? (
              <>
                ...
                <View style={styles.confirmRow}>
                  ...
                  >
                    <Text style={styles.destructiveText}>{busy ? "Deleting…" : "Delete"}</Text>
                  </Pressable>
                </View>
              </>
            ) : (
                ...
                {actions.map((action) => (
                  ...
                  >
                    <View
                      style={[
                        styles.iconCircle,
                        { backgroundColor: action.destructive ? `${colors.error}22` : colors.background },
                      ]}
                    >
                      <MaterialIcons
                        name={action.icon}
            ...
            <Pressable disabled={busy} onPress={close} style={styles.cancel}>
...
const styles = StyleSheet.create({
  ...
  title: { fontSize: 20, lineHeight: 26, fontWeight: "800", marginBottom: 6 },
  ...
  actionText: { flex: 1, fontSize: 15, lineHeight: 22, fontWeight: "600" },
  pressed: { opacity: 0.7 },
